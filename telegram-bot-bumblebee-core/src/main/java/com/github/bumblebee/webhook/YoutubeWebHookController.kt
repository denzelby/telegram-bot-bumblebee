...
@RestController
class YoutubeWebHookController(
    private val processor: YoutubeUpdateProcessor,
    private val parser: AtomParser
) {

    @RequestMapping(
        method = [(RequestMethod.POST)], path = ["/youtube"],
    ...
    fun confirmSubscription(
        ...
        @RequestParam("hub.lease_seconds") leaseSeconds: Int?
    ): String {
        return challenge
    }
